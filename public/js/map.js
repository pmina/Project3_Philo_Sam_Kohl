...
function getComments(eventId, map, callback) {
  ...
  $.get("/api/comments/" + eventId, function(response) {
    ...
    for (var count = 0; count < comments.length; count++) {
      ...
      $("#commentsList").prepend('<p class="commentlistclass">' + "<b>Location: </b>" + comment.location + "  <b></br>  Comment:    </b>" + comment.comment_data +  "  <b><br>  Time:    </b>" + moment(comment.createdAt).format(' h:mm a, MMM Do YY') + '</p>' )
    // =========================================================  
